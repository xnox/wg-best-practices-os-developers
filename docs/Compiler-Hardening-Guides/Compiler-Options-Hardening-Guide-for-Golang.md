# Compiler Options Hardening Guide for Go

*by the [Open Source Security Foundation (OpenSSF)](https://openssf.org) [Best Practices Working Group](https://best.openssf.org/), 2024-09-05*

This document is a guide for compiler and linker options that contribute to delivering reliable and secure code using native (or cross) toolchains for Go. The objective of compiler options hardening is to produce application binaries (executables) with security mechanisms against potential attacks and/or misbehavior. Furthermore to ensure optimised, reproducible binaries that are contain necessory metadata for security scanners.
...
- Those who write Go code, to help them ensure that resulting code will work with hardened options, including for embedded devices, Internet of Things devices, smartphones, and personal computers.
- Those who build Go code for use in production environments, including Linux distributions, device makers, and those who compile Go for their local environment.

This document focuses on recommended options for the Golang Go compiler. At the momemnt this guide focuses on builds for Linux, but will be expanded to cover Mac OS X and Windows targets in the future.

## TL;DR: What should I do?

When compiling Go code, use the following command:

~~~sh
CGO_ENABLED=1
GOAMD64=v2
GOARM64=v8.0
# unset GOEXPERIMENT or GOEXPERIMENT=none
go
-buildmode=pie
-ldflags "-w" # do not set -s
-tags netgo,osusergo
-trimpath
~~~

Also bump go version to latest available in `go.mod` file.

Also set `CGO_CFLAGS`, `CGO_CPPFLAGS`, `CGO_CXXFLAGS` and `CGO_LDFLAGS` as per the [Compiler Options Hardening Guide for C and C++].

[Compiler Options Hardening Guide for C and C++]: https://best.openssf.org/Compiler-Hardening-Guides/Compiler-Options-Hardening-Guide-for-C-and-C++.html

To preserve symbols tables for `govulnchecker` functionality: do not set `-ldflags "-s"` option, and do not execute `strip --strip-all` on the binaries.

## Background

### Why do we need compiler options hardening for Go?

The general principles of compiler hardening options are discussed in the original [Compiler Options Hardening Guide for C and C++]. Whilst many built-in features of the Go language address many hardening options by default, many optional features can still be enabled and enforced. Especially for Go applications that utilise C and C++ extensions, often the recommended hardening flags are missing[^missing-flags]. Out of backwards compatability concerns many new optimisations and hardening features of the go compiler are only possible to enable with environment flags and options.

[^missing-flags]: For example, default invocation of `dpkg-buildflags` do not export `CGO_CFLAGS` and similar.

### But my code doesn't call out to C?

Whilst many go applications are written in pure Go, none-the-less transitive dependencies and the standard go library may call into CGO. Furthermore, many TLS and cryptography related features are often initially can only enabled with upgraded `go.mod` or `GODEBUG` flags. More so, when attempting to achieve FIPS compliance by using `GOEXPERIMENT=systemcrypto` all of crypto module is replaced with CGO calls into OpenSSL and thus also requires hardening.

## Recommended Compiler Options

This section describes recommendations for compiler and linker option flags and environment variables that enable optimisations and binaries observibility. Also alternative flags and options are discussed when targetting specialised environments or builds.

Table 1: Environment Variables and flags

| Compiler Environment Variable or flag           |       Supported since       | Description                                                                         |
|:----------------------------------------------------------------------------- |:------------------------:|:----------------------------------------------------------------------------------- |
| GOAMD64=v2             | go1.18 | Enable AMD64 / X86_64 optimisations                                         |
| GOARM64=v8.0           | go1.23 | Enable ARM optimisations                                                 |
| CGO_CFLAGS=<see guide>    |        | C and C++ flags to pass to underlying compiler |
| -buildmode=pie         | go1.10 | Control build and link mode of binaries |
| -ldflags "-w"          |        | Recommended linker flags |
| -tags netgo,osusergo   |        | Recommended standard library build tags |
| -trimpath              |        | Reproducibile builds |
